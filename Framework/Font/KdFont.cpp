	...
	while(1)
	{
		...
		if (pFontDataArray)
		{
			if ((*pFontDataArray)[code])
			{
				...
				auto cachedData = (*pFontDataArray)[code];
				m_TexList.push_back(cachedData);
				m_TotalWidth += cachedData->AdvanceWidth;
			}
		}
		...
		if (bCreate)
		{
			...
			// フォントビットマップ取得
			//-------------------------------
			TEXTMETRIC TM;//フォント全体の寸法情報(例：フォントの標準の高さ、ベースラインの位置など）
			GetTextMetrics(hdc , &TM );//「hdc（デバイスコンテキスト）で現在設定されているフォントの情報をください」とOSにお願いする命令
			GLYPHMETRICS GM;//「1文字（グリフ）」のより詳細な寸法情報を格納(「文字の黒い部分（Black Box）の幅と高さ」や「描画開始位置のオフセット」「次の文字までの移動量」など)
			CONST MAT2 Mat = {{0,1},{0,0},{0,0},{0,1}};//単位行列
			DWORD size = GetGlyphOutline(hdc, code, gradFlag, &GM, 0, NULL, &Mat);	//OSに「この文字(code)の形を、この品質(gradFlag)で欲しいんだけど...」と伝える。アンチエイリアスの時に、spaceとか0が返るのはなぜ…
			std::unique_ptr<BYTE[]> ptr(new BYTE[size]);
			GetGlyphOutline(hdc, code, gradFlag, &GM, size, ptr.get(), &Mat);//ptr が指すメモリ領域に、文字の形（ビットマップデータ）を実際に書き込みます

			int addX = 0;//フォントのデザインによっては、'j' や 'f' の一部が、基準となる原点より左側にはみ出して描画されることがあるため右にずらす
			if(GM.gmptGlyphOrigin.x < 0)
			{
				addX = -GM.gmptGlyphOrigin.x;//gmptGlyphOriginは「文字のビットマップ（黒い部分）を描き始めるX座標」を示します
			...
			int fontWidth_Alignment4	= (GM.gmBlackBoxX + 3) / 4 * 4;//メモリへの高速アクセスのために4の倍数に切り上げ

			// 文字のバイト数
			if(b2byte)data->Bytes		= 2;
			else data->Bytes			= 1;

			data->AdvanceWidth = texWidth;
			...
			ZeroMemory(buf.get(), texWidth*texHeight* PIXEL_BYTES);
			...
			if (size > 0)
			{
				int sx, sy, dx, dy;
				DWORD alpha;
				int bBroken = 0;//理論上の計算値（total）と一致しない場合に立てるフラグ
				int bytePos;
				...
				if (gradFlag == GGO_BITMAP)
				{
					...
					int total = GM.gmBlackBoxY*iUseBYTEparLine;//(高さ) * (1行のバイト数) で、本来あるべき理論上の総データサイズを計算
					// 半分になってる場合
...
int KdFontManager::GetFontHeight(int fontNo)
{
	if (fontNo < 0 || fontNo >= m_FontTbl.size())
	{
		return 0; //範囲外
	}

	return m_FontTbl[fontNo].Height;
}

Math::Vector2 KdFontManager::MeasureString(int fontNo, const std::string& text, int antiAliasing)
{
	//高さを取得
	float height = static_cast<float>(GetFontHeight(fontNo));
	if (height == 0.0f) { return Math::Vector2(0, 0); }

	//幅取得
	std::shared_ptr<KdFontSprite> fontSprite = CreateFontTexture(fontNo, text, antiAliasing);

	if (!fontSprite) { return Math::Vector2(0.0f, 0.0f); }

	float width = static_cast<float>(fontSprite->GetTotalWidth());
