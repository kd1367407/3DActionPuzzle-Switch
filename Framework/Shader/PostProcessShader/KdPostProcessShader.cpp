...
void KdPostProcessShader::Resize(int w, int h)
{
	char buf[256];
    sprintf_s(buf, "PostProcessResize: w=%d, h=%d\n", w, h);
    OutputDebugStringA(buf);

	if (!KdDirect3D::Instance().WorkDev())return;

	//既存のレンダーターゲットを解放して作り直す
	m_postEffectRTPack.CreateRenderTarget(w, h, true);
	m_blurRTPack.CreateRenderTarget(w, h, true);
	m_strongBlurRTPack.CreateRenderTarget(w / 2, h / 2);
	m_depthOfFieldRTPack.CreateRenderTarget(w, h);
	m_brightEffectRTPack.CreateRenderTarget(w, h);

	int lightBloomWidth = w;
	int lightBloomHeight = h;
