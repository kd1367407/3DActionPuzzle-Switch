...
public:
	...
	//KdFont描画(低レベル関数)
	void	DrawFont(int fontNo, const std::string& text, float x, float y, const Math::Color* color, int antiAliasing = 0);

	void DrawFont(int fontNo, const std::string& text, const Math::Vector2& pos, const Math::Color* color, int antiAliasing = 0)
	{
		DrawFont(fontNo, text, pos.x, pos.y, color, antiAliasing);
	}
