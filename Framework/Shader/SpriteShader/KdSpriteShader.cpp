...
void KdSpriteShader::DrawFont(int fontNo, const std::string& text, float x, float y, const Math::Color* color, int antiAliasing)
{
	// もし開始していない場合は開始する(最後にEnd())
	bool bBgn = m_isBegin;
	if (!bBgn)Begin();
	...
	lineHeight = static_cast<float>(KdFontManager::Instance().GetFontHeight(fontNo));
	...
	while (std::getline(ss, line, '\n'))
	{
		...
		//1行文のフォントスプライトをマネージャーから取得
		std::shared_ptr<KdFontSprite> fontSprite = KdFontManager::Instance().CreateFontTexture(fontNo, line, antiAliasing);

		if (fontSprite && !fontSprite->GetTexList().empty())
		{
			for (auto& data : fontSprite->GetTexList())
			{
				if (!data || !data->FontTex)continue;

				std::shared_ptr<KdTexture> tex = data->FontTex;

				DrawTex(tex.get(), static_cast<int>(cursorX), static_cast<int>(cursorY), tex->GetInfo().Width, tex->GetInfo().Height, nullptr, color, Math::Vector2(0.0f, 0.0f));

				cursorX += tex->GetInfo().Width;
			}
		}
		//カーソル改行
		if (lineHeight > 0)
