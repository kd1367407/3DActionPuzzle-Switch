...
void KdStandardShader::BeginLit()
{
	...
	if (KdShaderManager::Instance().SetVertexShader(m_VS_Lit))
	{
		...
		//スキンメッシュ対応
		KdShaderManager::Instance().SetVSConstantBuffer(4, m_cb4_Bone.GetAddress());
	}
	...
	const Math::Color& colRate, const Math::Vector3& emissive)
{
	...
	if (data->IsSkinMesh())
	{
		for (auto nodeIdx : data->GetBoneNodeIndices())
		{
			...
			//ボーン行列からGPUに渡す行列の計算
			m_cb4_Bone.Work().mBones[dataNode.m_boneIndex] = dataNode.m_boneInverseWorldMatrix * workNode.m_worldTransform;
		}
...
#include "KdStandardShader_VS_Lit.shaderInc"
		...
		std::vector<D3D11_INPUT_ELEMENT_DESC> layout = {
			...
			{"SKINWEIGHT",0,DXGI_FORMAT_R32G32B32A32_FLOAT,0,56,D3D11_INPUT_PER_VERTEX_DATA}
		};
...
void KdStandardShader::WriteMaterial(const KdMaterial& material, const Math::Vector4& colRate, const Math::Vector3& emiRate)
{
	...
	if (emiRate.x != 0.0f || emiRate.y != 0.0f || emiRate.z != 0.0f)
	{
		m_cb2_Material.Work().Emissive = emiRate;
	}
	else
	{
