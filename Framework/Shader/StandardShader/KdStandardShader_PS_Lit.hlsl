...
float4 main(VSOutput In) : SV_Target0
{
	...
	if (g_dissolveResolution>0.0f)
	{
		//整数にしてから割ってブロック状にする
		uv = floor(uv * g_dissolveResolution) / g_dissolveResolution;
	}
	
	float discardValue = g_dissolveTex.Sample(g_ss, uv).r;
    ...
    float3 texEmissive = g_emissiveTex.Sample(g_ss, In.UV).rgb;
	
    float3 finalEmissive = texEmissive * g_Emissive;

    float alpha = max(baseColor.a, texEmissive.r);
	
	// Alphaテスト
	if (alpha < 0.05f && !g_OnlyEmissie)
	{
		discard;
	}
	...
	if (g_OnlyEmissie)
	{
		...
		//C++から渡された g_Emissive (m_currentEmissiveColor) で色を上書き
		//(g_emissiveTex(白) は無視し、線の色(baseColor.rgb)も無視する)
		outColor = g_Emissive * In.Color.rgb;

		//描画するピクセルは強制的に不透明(1.0)にする
		alpha = 1.0f;
