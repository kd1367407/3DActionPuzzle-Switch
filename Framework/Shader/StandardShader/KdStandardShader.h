...
public:
	...
	// ディゾルブ設定
	void SetDissolve(float threshold, const float* range = nullptr, const Math::Vector3* edgeColor = nullptr, const float* resolution = nullptr)
	{
		...
		if (resolution)
		{
			cbObj.DissolveResolution = *resolution;
		}
	...
	const	 cbBone& WorkBoneCB()const { return m_cb4_Bone.Get(); }
...
private:
	...
	{
		if (m_cb0_Obj.Work().IsSkinMeshObj != (isSkinMesh ? 1 : 0))
		{
			m_cb0_Obj.Work().IsSkinMeshObj = (isSkinMesh ? 1 : 0);
			m_dirtyCBObj = true;
		}
	}
	...
	KdConstantBuffer<cbBone> m_cb4_Bone;
