...
//	std::wstring wndClsName = sjis_to_wide(windowClassName.data());
	...
	m_hWnd = CreateWindow(
		...
		WS_OVERLAPPEDWINDOW,								// ウィンドウをリサイズ可能なウィンドウへ
		0,													// ウィンドウの位置（Ｘ座標）
		0,													// ウィンドウの位置（Ｙ座標）						
		clientWidth,										// ウィンドウの幅
...
LRESULT KdWindow::WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		...
		// ウィンドウ破棄直前
	case WM_DESTROY:
		//TEXTを付けるとUnicode設定でもマルチバイト設定でも勝手に書き換えてくれる
		RemoveProp(hWnd, TEXT("GameWindowInstance"));
		PostQuitMessage(0);
		break;
	case WM_SYSKEYDOWN:
	...
	case WM_SIZE:
		if (wParam != SIZE_MINIMIZED) {
			...
			if (newW && newH) {
				// バックバッファとホストプロセス用バッファもリサイズ
				if (KdDirect3D::Instance().Resize((int)newW, (int)newH))
				{
					KdShaderManager::Instance().Resize((int)newW, (int)newH);
				}
			}
		}
		break;
